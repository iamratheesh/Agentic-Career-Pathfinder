...
# (No changes from previous version, it will automatically use Groq API key via settings)

from fastapi import APIRouter, HTTPException
from motor.motor_asyncio import AsyncIOMotorClient
from database import get_database
from models import CareerTrack, SessionDocument, CareerTrackDocument
...
@router.get("/career-tracks/{session_id}", response_model=List[CareerTrack])
async def get_career_tracks(session_id: str):
    ...
    # This agent expects 'api_key' which will now be the GROQ_API_KEY from settings
    recommender_agent = CareerTrackRecommenderAgent(
        api_key=settings.GROQ_API_KEY, # Explicitly pass GROQ_API_KEY here for clarity
        tavily_api_key=settings.TAVILY_API_KEY
    )
    
    try:
        recommended_tracks = await recommender_agent.recommend_tracks(domain, level)
    ...
    return [CareerTrack(**track) for track in recommended_tracks]